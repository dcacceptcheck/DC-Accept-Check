...
function readRow(row) {
  ...
  const criteriaDaysRaw = findValueByContains(row, ["เกณฑ์", "กี่วัน"]) ??
    findValueByContains(row, ["ต้องไม่เกินกี่วัน"]);

  const criteriaPercent = toNumber(
    findValueByExact(row, ["เกณฑ์การรับสินค้า ของดีซีห้าง ต้องไม่เกินกี่ % นับจากวันผลิต"]) ??
    findValueByContains(row, ["เกณฑ์", "%"]) ??
    findValueByContains(row, ["กี่%"])
  );

  let criteriaDays = toNumber(criteriaDaysRaw);

  // Support updated Excel formula in Column F:
  // =IF(สินค้า="สด",ROUNDDOWN(อายุสินค้า*% ,0)-3,ROUNDDOWN(อายุสินค้า*% ,0)-7)
  // The browser reads formula cells as text, so calculate the same rule here.
  if (criteriaDays === null && shelfLifeDays !== null && criteriaPercent !== null) {
    const baseDays = Math.floor(shelfLifeDays * criteriaPercent);
    criteriaDays = product === "สด" ? baseDays - 3 : baseDays - 7;
  }
